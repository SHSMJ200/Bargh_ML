...
def plot():
    ...
    for row in power_plants.itertuples():
        ...
        ds_n_c_plot = Data_selector(Data_selector(df).select_peaks(goodness=0, is_tight=False))
        df_n_c_plot = ds_n_c_plot.filter_name_code(name, code)
        
        try:
            draw_gen_temp_plot(df_n_c_plot, coefs, name, code)
        except Exception as e:
            print(f"This error occurred while drawing diagram related to {name}-{code}-temp:\n {e}")
        
        try:
            draw_gen_date_plot(df_n_c_plot, name, code)
        except Exception as e:
            print(f"This error occurred while drawing diagram related to {name}-{code}-date :\n {e}")    
