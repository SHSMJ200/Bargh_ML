...
def add_features_and_filter(df, l_min, max_diff, c_thresh, bin_length, temp_feature):
    ...
    feature_adder.filter3(l_min=l_min, max_diff=max_diff, initial_label=1)
    # feature_adder.filter3(c_thresh=c_thresh, initial_label=2)
    feature_adder.filter5(initial_label=2)
    feature_adder.filter6(bin_length, initial_label=4)
